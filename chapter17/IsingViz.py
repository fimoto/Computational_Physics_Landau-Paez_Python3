...
def spstate(state, time):                                        # Plots spins
    ...
    for i in range(-N,N,2):
        ...
        if  5*state[j]<0:
            ...
            x1list.append(i/2.+N/2)
            y1list.append(time)
        else:
            x2list.append(i/2.+N/2)
            y2list.append(time)
            #arrowcol = 'black'
...
for  j in range (1,1000):      #500
      ...
      test[r] *= -1                  
      ET = energy(test)              
      p = exp((ES-ET)/(k*T))   #  Boltzmann test
      ...
      if p >= random.random():       
           state = test
           spstate(state,time)
           im1 = plt.plot(y1list,x1list,'o',color ='blue',markersize=2)
           im2 = plt.plot(y2list,x2list,'o',color ='white',markersize=2)
           ims1.append(im1)
           ims2.append(im2)
           """
           x1list.clear()
